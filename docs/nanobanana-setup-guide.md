# NanoBanana Pro Setup Guide (Ultimate Edition)

This guide documents the strict configuration for the HuePress "Art Factory" using Gemini 3 Pro (NanoBanana Pro).

## 1. Model Configuration

Set these parameters in the model sidebar:

- **Model:** `gemini-3-pro-image-preview` (NanoBanana Pro)
- **Temperature:** `1.0` (Balances creativity with strict adherence)
- **Aspect Ratio:** `Auto` (Controlled via System Prompt to A4)
- **Resolution:** `4K` (Required for clean vectorization)
- **Output Length:** `32768`

## 2. System Instructions (The Brain)

**Copy and paste this ENTIRE JSON object into the "System instructions" field.** This acts as the immutable "Art Director" for the model.
...
## 3. User Input (The Trigger)

Paste the Asset JSON provided by the HuePress database. The model will parse it against the System Instructions above.

**Example Input:**

```json
{
  "title": "Space Explorer With Pet Bot",
  "description": "A space explorer floats near a friendly pet robot...",
  "category": "Pop Culture",
  "skill": "Medium",
  "tags": "space-explorer, pet-bot...",
  "extendedDescription": "...",
  "funFacts": "...",
  "suggestedActivities": "..."
}
